...
New subproject `realty-paper-adapters/pn-adapter`, added to `settings.gradle.kts`,
...
`module-manifest.yml`: `module-name: pn-adapter`, `reloadable: true`.
...
2. **`Realty.startModules()`** — remove the `BundledModuleExtractor.extract(...)` call and
   ...
   - The "no modules at all" warning names `pn-adapter.jar` alongside `chat-adapter.jar`.
   - The chat-adapter-specific warning stays quiet when `pn-adapter` is loaded, so a
...
3. **`runServer`** — keep staging `chat-adapter.jar` (a dev convenience independent of
   shipping; removing it breaks local smoke tests) and add `pn-adapter.jar` to the same
   `doFirst` staging. PN itself is not downloaded by `runServer`, so `pn-adapter` will fail
